...
manual_tests2 <- function() {
  ...
  mms <- lapply(setNames(nm = stanfiles), function(sf) {
    ...
    msp <- specs(sf)
    mdat <- if (
      mm_parse_name(sf, expand = TRUE)$pool_K600_type %in% c('linear', 'binned')
    ) {
      dat
    } else {
      select(dat, -discharge)
    }
  ...
  kqfits <- lapply(mms, function(mm) {
    ...
    dailymean <- function(
      ...
      data.frame(
        dischdaily = if (isTRUE(ply_validity[1])) {
    ...
    dischdaily <- mm_model_by_ply(
      model_fun = dailymean,
      data = dat,
      day_start = mms[[1]]@specs$day_start,
      day_end = mms[[1]]@specs$day_end
    )
    ...
    switch(
      ...
      'none' = {
        ...
        ggplot(pars, aes(x = log(K600.daily))) +
          ...
          geom_errorbarh(
            aes(x = K$mean, y = 0, xmin = K$mean - K$sd, xmax = K$mean + K$sd),
            color = 'purple',
            height = 0.05
          ) +
          stat_function(
            fun = dnorm,
            args = list(mean = K$mean, sd = K$sd),
            color = "purple"
          ) +
          xlab('ln(K600)') +
          ylab('density')
        ...
        ggplot(pars, aes(x = log(K600.daily))) +
          ...
          geom_errorbarh(
            aes(
              x = K$mean,
              y = 0,
              xmin = K$mean - 1.96 * K$sd,
              xmax = K$mean + 1.96 * K$sd
            ),
            color = 'red',
            height = 0.05
          ) +
          stat_function(
            fun = dnorm,
            args = list(mean = K$mean, sd = 1.96 * K$sd),
            color = "red"
          ) +
          xlab('ln(K600)') +
          ylab('density')
      ...
      'linear' = {
        ...
        ggplot(
          parslnQ,
          aes(x = lnQ, y = lnK, ymin = lnK.lower, ymax = lnK.upper)
        ) +
          geom_abline(intercept = K$icpt, slope = K$slope, col = 'red') +
          geom_point(aes(y = lnK.pred), color = 'red') +
          geom_ribbon(
            aes(
              ymin = log(pmax(0.0001, exp(lnK.pred) - 1.96 * K$sd)),
              ymax = log(exp(lnK.pred) + 1.96 * K$sd)
            ),
            color = NA,
            fill = 'red',
            alpha = 0.2
      ...
      'binned' = {
        K <- tibble::tibble(
          ...
          lnKbin.lower = fit[[as.character(length(
            lnQ
          ))]]$lnK600_lnQ_nodes_2.5pct,
          lnKbin.upper = fit[[as.character(length(
            lnQ
          ))]]$lnK600_lnQ_nodes_97.5pct,
          sd = get_specs(mm)$K600_daily_sigma
        )
        lnQdat <-
          tibble::tibble(
          ...
          mutate(
            lnQ_daily = K$lnQ[lnQ_bin1] *
              lnQ_bin_weights1 +
              K$lnQ[lnQ_bin2] * lnQ_bin_weights2,
            lnK_pred = K$lnKbin[lnQ_bin1] *
              lnQ_bin_weights1 +
              K$lnKbin[lnQ_bin2] * lnQ_bin_weights2
          )
        parslnQ <- mutate(
          pars,
        ...
        ggplot(parslnQ, aes(x = lnQ)) +
          ...
          geom_ribbon(
            data = K,
            aes(
              ymin = log(pmax(0.0001, exp(lnKbin) - 1.96 * K$sd)),
              ymax = log(exp(lnKbin) + 1.96 * K$sd)
            ),
            color = NA,
            fill = 'red',
            alpha = 0.2
          ...
          geom_point(aes(y = lnK)) +
          geom_errorbar(aes(ymin = lnK.lower, ymax = lnK.upper)) +
          ylab('ln(K600)') +
          xlab('ln(Q)')
      }
    ) +
  ...
  do.call(
    grid.arrange,
    c(
      lapply(mms, function(mm) {
        plot_DO_preds(mm, y_var = 'pctsat') + ggtitle(mm@specs$model_name)
      }),
      list(nrow = 2, ncol = 2)
    )
  )
...
manual_tests <- function() {
  ...
  pi <- metab(
    specs(
      ...
      err_proc_iid_sigma_scale = 0.001
    ),
    dat
  ) # DO_obs currently required for pi models
  ...
  stan_trace(
    get_mcmc(oipi),
    pars = c('err_obs_iid_sigma', 'err_proc_iid_sigma')
  )
  ...
  sp <- mm_name('bayes', pool_K600 = 'normal') %>%
    specs(
      ...
      err_obs_iid_sigma_rate = 1,
      err_proc_iid_sigma_shape = 1,
      err_proc_iid_sigma_rate = 1,
      n_chains = 1,
      n_cores = 4,
      burnin_steps = 1500,
      saved_steps = 1500
  ...
  fitall <- metab_bayes(
    vfrenchmedium,
    specs = replace(ms, 'split_dates', FALSE)
  ) # all three days at once
