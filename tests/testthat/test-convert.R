test_that("converting between k600 and kgas works", {
  ...
  expect_equal(
    convert_k600_to_kGAS(
      convert_kGAS_to_k600(kGAS = 15, temperature = 15),
      temperature = 15
    ),
    15
  )
  expect_equal(
    convert_kGAS_to_k600(
      convert_k600_to_kGAS(k600 = 10, temperature = 15),
      temperature = 15
    ),
    10
  )
...
test_that("converting between date and DOY works", {
  ...
  expect_equal(
    ...
    150,
    info = "treat numbers as true time since jan 1, ignoring daylight time"
  )
  # doyhr to date
  expect_equal(
    streamMetabolizer:::convert_doyhr_to_date(1, year = 1920),
    as.POSIXct("1920-01-01 00:00:00", tz = "UTC"),
    info = "1 should be Jan 1, default UTC"
  )
  expect_equal(
    streamMetabolizer:::convert_doyhr_to_date(3, year = 2016, tz = "CST6CDT"),
    as.POSIXct("2016-01-03 00:00:00", tz = "CST6CDT"),
    info = "tz should stay as indicated"
  )
  expect_equal(
    streamMetabolizer:::convert_doyhr_to_date(150, year = 2016, tz = "CST6CDT"),
    as.POSIXct("2016-05-29 01:00:00", tz = "CST6CDT"),
    info = "treat numbers as true time since jan 1, ignoring daylight time"
  )
  # there and back
  expect_equal(
    streamMetabolizer:::convert_date_to_doyhr(streamMetabolizer:::convert_doyhr_to_date(
      12,
      year = 2024,
      tz = "CST6CDT"
    )),
    12,
    info = "should preserve DOY in there&back"
  )
  expect_equal(
    streamMetabolizer:::convert_date_to_doyhr(streamMetabolizer:::convert_doyhr_to_date(
      120,
    ...
    120,
    info = "should preserve DOY in there&back even during daylight savings"
  )
  expect_equal(
    streamMetabolizer:::convert_doyhr_to_date(
      streamMetabolizer:::convert_date_to_doyhr(as.POSIXct(
        "2007-05-29 01:00:00",
        tz = "America/Denver"
      )),
    ...
    as.POSIXct("2007-05-29 01:00:00", tz = "America/Denver"),
    info = "preserve date in there&back"
  )
})
...
test_that("converting between UTC and solar time works", {
  ...
  somedates <- seq(
    adate,
    adate + as.difftime(365 * 2, units = "days"),
    by = as.difftime(10.35, units = "days")
  )
  # UTC to solar
  expect_equal(
    convert_UTC_to_solartime(adate, longitude = 0, time.type = "mean solar"),
    adate
  )
  expect_equal(
  ...
  expect_lt(
    as.numeric(
      convert_UTC_to_solartime(
        ...
        (adate + as.difftime(7, units = "hours")),
      units = 'secs'
    ),
    10
  ) #go east, be later
  ...
  expect_equal(
    ...
    info = "negative degrees are degW"
  )
  expect_equal(
    convert_UTC_to_solartime(
      adate,
    ...
    info = "mix&match units is OK for this fun"
  )
  expect_equal(
    convert_UTC_to_solartime(
      somedates,
  ...
  # solar to UTC
  expect_equal(
    convert_solartime_to_UTC(adate, longitude = 0, time.type = "mean solar"),
    adate
  )
  expect_equal(
  ...
  expect_equal(
    as.numeric(convert_UTC_to_solartime(
      convert_solartime_to_UTC(adate, longitude = -103.8, time.type = "app"),
      longitude = -103.8,
      time.type = "apparent solar"
    )),
    as.numeric(adate),
  ...
  expect_equal(
    as.numeric(convert_solartime_to_UTC(
      convert_UTC_to_solartime(adate, longitude = -103.8, time.type = "app"),
      longitude = -103.8,
      time.type = "appar"
    )),
    as.numeric(adate),
...
test_that("converting between UTC and local time works", {
  ...
  expect_equal(
    convert_UTC_to_localtime(
      ...
      latitude = 51.48,
      longitude = 0,
      time.type = "standard local"
    ),
    adate
  )
  expect_equal(
    as.numeric(convert_UTC_to_localtime(
      adate,
      latitude = 51.48,
      longitude = 0,
      time.type = "daylight local"
    )),
    as.numeric(adate),
    info = "still the same #s"
  )
  expect_equal(
    lubridate::tz(convert_UTC_to_localtime(
      adate,
      latitude = 51.48,
      longitude = 0,
      time.type = "daylight local"
  ...
  expect_equal(
    ...
    "Etc/GMT-8",
    info = "go east, be POSIX-negative"
  )
  expect_equal(
    lubridate::tz(convert_UTC_to_localtime(
      adate,
    ...
    "Etc/GMT+7",
    info = "go west, be POSIX-positive"
  )
  expect_equal(
    lubridate::tz(convert_UTC_to_localtime(
      adate,
  ...
  expect_equal(
    ...
    lubridate::with_tz(somedates, "America/New_York"),
    info = "handle multiple dates"
  )
  expect_equal(
    convert_UTC_to_localtime(
    ...
    lubridate::with_tz(somedates, "Etc/GMT+5"),
    info = "handle multiple dates"
  )
  ...
  expect_equal(
    convert_localtime_to_UTC(lubridate::with_tz(adate, "Etc/GMT+8")),
    adate
  )
  expect_equal(
    convert_localtime_to_UTC(lubridate::with_tz(adate, "Etc/GMT-5")),
    adate
  )
  ...
  expect_equal(
    convert_UTC_to_localtime(
      convert_localtime_to_UTC(lubridate::with_tz(adate, "America/Denver")),
      latitude = 40,
      longitude = -105.3,
      time.type = "daylight"
    ),
    lubridate::with_tz(adate, "America/Denver")
  )
  expect_equal(
    convert_localtime_to_UTC(convert_UTC_to_localtime(
      adate,
      latitude = 40,
      longitude = -105.3,
      time.type = "daylight"
...
test_that("common use-case conversions (calc_solar_time) works", {
  ...
  expect_equal(
    calc_solar_time(adate, -120),
    calc_solar_time(lubridate::with_tz(adate, 'America/Anchorage'), -120)
  )
  expect_equal(
