...
test_that("French Creek predictions are similar for streamMetabolizer & Bob Hall's code", {
  ...
  start.posix <- as.POSIXct(
    format(start.chron, "%Y-%m-%d %H:%M:%S"),
    tz = "UTC"
  )
  end.posix <- as.POSIXct(format(end.chron, "%Y-%m-%d %H:%M:%S"), tz = "UTC")
  mid.date <- as.Date(start.posix + (end.posix - start.posix) / 2)
  start.numeric <- as.numeric(
    start.posix - as.POSIXct(format(mid.date, "%Y-%m-%d 00:00:00"), tz = "UTC"),
    units = 'hours'
  )
  end.numeric <- as.numeric(
    end.posix - as.POSIXct(format(mid.date, "%Y-%m-%d 00:00:00"), tz = "UTC"),
    units = 'hours'
  )

  # get, format, & subset data
  vfrench <- streamMetabolizer:::load_french_creek(attach.units = FALSE)
  vfrenchshort <- vfrench[
    vfrench$solar.time >= start.posix & vfrench$solar.time <= end.posix,
  ]
  ...
  stop.dark <- 100 +
    which(
      format(vfrenchshort$solar.time[101:nrow(vfrenchshort)], "%H:%M") ==
        "23:00"
    )[1]
  vfrenchnight <- vfrenchshort[first.dark:stop.dark, ]
  night.start <- eval(parse(
    text = format(vfrenchnight$solar.time[1], "%H + %M/60")
  ))
  night.end <- eval(parse(
    text = format(vfrenchnight$solar.time[nrow(vfrenchnight)], "%H + %M/60")
  ))
  ...
  bobest <- streamMetabolizer:::load_french_creek_std_mle(
    vfrenchnight,
    estimate = 'K',
    start = chron::chron(dates = "08/24/12", times = "18:45:00"),
    end = chron::chron(dates = "08/24/12", times = "22:56:00")
  )
  ...
  mm <- metab_mle(
    ...
    data = vfrenchshort,
    data_daily = data.frame(date = mid.date, K600.daily = 35)
  )
