...
test_that('get_params options are honored (for MLE models): uncertainty', {
  ...
  expect_equal(
    grep('\\.sd$', names(ps), value = TRUE),
    c('GPP.daily.sd', 'ER.daily.sd', 'K600.daily.sd')
  )
  ps <- get_params(mm, uncertainty = 'ci')
  expect_equal(
    grep('\\.lower$', names(ps), value = TRUE),
    c('GPP.daily.lower', 'ER.daily.lower', 'K600.daily.lower')
  )
...
test_that('get_params options are honored (for MLE models): fixed', {
  ...
  expect_equal(
    grep('\\.fixed$', names(ps), value = TRUE),
...
test_that('get_params options are honored (for MLE models): uncertainty+fixed', {
  ...
  expect_equal(
    grep('\\.lower$', names(ps), value = TRUE),
    c('GPP.daily.lower', 'ER.daily.lower')
  )
  expect_equal(grep('\\*', ps), match('K600.daily', names(ps)))
...
test_that('get_params works for each model type, basic GPP & ER equations', {
  ...
  mm <- metab_sim(
    specs(mm_name('sim'), K600_daily = NULL, GPP_daily = NULL, ER_daily = NULL),
    data = dat,
    data_daily = dat_daily
  )
  ps <- get_params(mm)
  ...
  expect_equal(
    grep('\\*', ps),
    match(c('K600.daily', 'GPP.daily', 'ER.daily'), names(ps))
  )
  dat_daily <- data.frame(
    date = as.Date(paste0("2012-09-", 18:20)),
    Pmax = 6,
