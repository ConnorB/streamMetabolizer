test_that("metab_sim predictions (predict_metab, predict_DO) make sense", {
  ...
  mm3 <- metab_sim(
    revise(sp, day_tests = c()),
    data = select(dat, -DO.obs),
    data_daily = dd
  )
  expect_equal(
  ...
  mmE <- metab_sim(
    revise(sp, model_name = 's_np_oipcpi_eu_plrckm.rnorm'),
    data = dat,
    data_daily = dd
  )
  mmP <- metab_sim(
    revise(sp, model_name = 's_np_oipcpi_tr_plrckm.rnorm'),
    data = dat,
    data_daily = dd
  )
  rmseEP <- sqrt(mean(
    (predict_DO(mmE)$DO.obs - predict_DO(mmP)$DO.obs)^2,
    na.rm = TRUE
  ))
  ...
  diffs <- mm_model_by_ply(
    function(data_ply, ...) {
      data.frame(
        DO.mod.1 = head(data_ply$DO.mod, 1),
        DO.mod.n = tail(data_ply$DO.mod, 1)
      )
    },
    data = predict_DO(msim),
    day_start = msim@specs$day_start,
