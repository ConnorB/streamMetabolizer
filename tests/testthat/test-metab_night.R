...
test_that("metab_night predictions (predict_metab, predict_DO) make sense", {
  ...
  # 10 days
  mm <- metab_night(
    data = data_metab('10', res = '5', day_start = 12, day_end = 36)
  )
  metab <- predict_metab(mm)
  DO_preds <- predict_DO(mm)
  expect_lt(rmse_DO(DO_preds), 0.1)
...
test_that("day_tests=c('full_day','include_sunset') get handled appropriately", {
  ...
  mm <- metab_night(
    specs(mm_name('night'), day_start = 12, day_end = 35),
    data = data_metab('1', day_start = 12, day_end = 35)
  )
  ...
  sp <- specs(
    ...
    day_start = 12,
    day_end = 35,
    day_tests = c('include_sunset', 'full_day')
  )
  dat <- data_metab('1', day_start = 22, day_end = 35) # start the day too late
  expect_equal(
    get_fit(metab_night(
      replace(sp, 'day_tests', 'full_day'),
      data = dat
    ))$errors,
  ...
  expect_equal(
    get_fit(metab_night(sp, data = dat))$errors,
    "data don't include day-night transition; data don't start when expected"
  )
...
test_that("metab_night predictions match Bob's", {
  ...
  mmb <- streamMetabolizer:::load_french_creek_std_mle(
    dat,
    estimate = 'K',
    start = c(dates = "09/18/12", times = "18:00:00"),
    end = c(dates = "09/19/12", times = "05:00:00")
  )
