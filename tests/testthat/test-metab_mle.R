...
test_that("metab_mle predictions (predict_metab, predict_DO) make sense", {
  # 1 day
  mm <- metab_mle(
    specs(mm_name('mle'), day_start = -1, day_end = 23),
    data = data_metab('1', day_start = -1, day_end = 23)
  )
  ...
  # 10 days
  mm <- metab_mle(
    specs(mm_name('mle'), day_start = 2, day_end = 26),
    data = data_metab('10', day_start = 2, day_end = 26)
  )
  metab <- predict_metab(mm)
  DO_preds <- predict_DO(mm)
  expect_true(
  ...
  K600 <- dplyr::transmute(
    predict_metab(mmP),
    date = date,
    K600.daily = c(NA, 20, 21)
  )
...
test_that("metab_mle outputs look like Bob's", {
  ...
  # PRK
  mms <- metab_mle(
    specs(mm_name('mle', ode_method = 'euler'), day_start = -2, day_end = 30),
    data = dat
  )
  ...
  mms <- metab_mle(
    specs(mm_name('mle', ode_method = 'euler'), day_start = -2, day_end = 30),
    data = dat,
    data_daily = data.frame(date = as.Date("2012-09-18"), K600.daily = 35)
  )
  mmb <- streamMetabolizer:::load_french_creek_std_mle(
    dat,
    estimate = 'PR',
...
test_that("metab_models can be saved & reloaded efficiently (see helper-save_load.R)", {
  skip(
    "skipping check of gz9 as a great file compression choice; just doesn't seem that important"
  )
