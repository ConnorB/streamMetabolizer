...
roundtrip <- function(dat, con_fun, reps = 10, ...) {
  ...
  save <- summary(
    microbenchmark::microbenchmark(saveRDS(dat, con), times = reps),
    unit = 'ms'
  )$mean
  load <- summary(
    microbenchmark::microbenchmark(x <- readRDS(test)),
    unit = 'ms'
  )$mean
...
save_load_timing <- function(dat, reps = 10, ...) {
  dplyr::bind_rows(
    ...
    data.frame(
      type = 'gz',
      level = 1,
      roundtrip(dat, gzfile, reps = reps, compression = 1),
      stringsAsFactors = FALSE
    ),
    data.frame(
      type = 'gz',
      level = 6,
      roundtrip(dat, gzfile, reps = reps, compression = 6),
      stringsAsFactors = FALSE
    ),
    data.frame(
      type = 'gz',
      level = 9,
      roundtrip(dat, gzfile, reps = reps, compression = 9),
      stringsAsFactors = FALSE
    ),
    data.frame(
      type = 'bz',
      level = 1,
      roundtrip(dat, bzfile, reps = reps, compression = 1),
      stringsAsFactors = FALSE
    ),
    data.frame(
      type = 'bz',
      level = 6,
      roundtrip(dat, bzfile, reps = reps, compression = 6),
      stringsAsFactors = FALSE
    ),
    data.frame(
      type = 'bz',
      level = 9,
      roundtrip(dat, bzfile, reps = reps, compression = 9),
      stringsAsFactors = FALSE
    ),
    data.frame(
      type = 'xz',
      level = 1,
      roundtrip(dat, xzfile, reps = reps, compression = 1),
      stringsAsFactors = FALSE
    ),
    data.frame(
      type = 'xz',
      level = 6,
      roundtrip(dat, xzfile, reps = reps, compression = 6),
      stringsAsFactors = FALSE
    ),
    data.frame(
      type = 'xz',
