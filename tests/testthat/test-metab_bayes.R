...
manual_test4 <- function() {
  ...
  mms <- lapply(setNames(nm = stanfiles[7]), function(sf) {
    message(sf)
    sp <- revise(
      specs(sf, burnin_steps = 100, saved_steps = 50),
      params_out = union(params_out, c('err_obs_iid', 'err_proc_iid'))
    )
    metab(sp, dat)
  })
  ...
  mm_files <- dir(
    path = '../metab_tests/explore/007_stan_tests/',
    pattern = '16.*',
    full.names = TRUE
  )
  ...
  do.call(
    grid.arrange,
    c(
      lapply(mms[c(1, 4, 2, 3)], function(mm) {
        plot_DO_preds(mm) + ggtitle(mm@specs$model_name)
      }),
      list(nrow = 2, ncol = 2)
    )
  )
  do.call(
    ...
    c(
      lapply(mms[c(1, 4, 2, 3)], function(mm) {
        plot_metab_preds(mm) + ggtitle(mm@specs$model_name)
      }),
      list(nrow = 2, ncol = 2)
    )
  )
  do.call(
    grid.arrange,
    c(
      lapply(mms[c(1, 5, 9, 2, 6, 10, 3, 7, 11, 4, 8, 12)], function(mm) {
        plot_DO_preds(mm) + ggtitle(mm@specs$model_name)
      }),
      list(nrow = 4, ncol = 3)
    )
  ...
  do.call(
    ...
    c(
      lapply(mms[c(1, 5, 9, 2, 6, 10, 3, 7, 11, 4, 8, 12)], function(mm) {
        traceplot(get_mcmc(mm), 'K600_daily_mu') + ggtitle(mm@specs$model_name)
      }),
      list(nrow = 4, ncol = 3)
    )
  )
...
manual_test1 <- function() {
  ...
  test_that("simple bayesian models run and implement the interface", {
    ...
    expect_equal(
      1,
      length(grep(
        "SAMPLING FOR MODEL 'b_np_oi_tr_plrckm' NOW",
        get_log(mm)$MCMC_All_Days
      ))
    )
  ...
  sp <- function(split_dates) {
    replace(
      specs(
        mm_name('bayes', err_proc_iid = FALSE),
        n_cores = 3,
        n_chains = 3,
        burnin_steps = 300,
        saved_steps = 200,
  ...
  test_that("error and warning messages are printed with the mm object if present", {
    ...
    expect_equal(
      get_params(metab(sp(FALSE), dat))$errors,
      c('', 'uneven timesteps', '')
    )
    expect_equal(
      get_params(metab(sp(TRUE), dat))$errors,
      c('', 'uneven timesteps', '')
    )
  })
}
...
manual_test2 <- function() {
  ...
  mm <- metab(
    revise(
      specs("b_np_oi_tr_plrckm.stan"),
      params_out = c(params_out, 'DO_mod')
    ),
    dat
  )
  mm <- metab(
    revise(
      specs("b_np_oi_tr_plrcko.stan"),
      params_out = c(params_out, 'DO_mod')
    ),
    dat
  )
...
manual_test3 <- function() {
  ...
  pairs(
    get_mcmc(mm_old),
    pars = c("GPP_daily[7]", "ER_daily[7]", "K600_daily[7]")
  )
  sp <- specs(
    "b_Kl_pcpi_tr_plrcko.stan",
    n_chains = 3,
    ...
    revise(
      ...
      err_proc_acor_phi_rate = 50000,
      err_proc_acor_sigma_rate = 0.001,
      err_proc_iid_sigma_rate = 0.02,
      params_in = c(
        'GPP_daily_mu',
        'GPP_daily_sigma',
        'ER_daily_mu',
        'ER_daily_sigma',
        'K600_daily_beta_mu',
        'K600_daily_beta_sigma',
        'K600_daily_sigma_rate',
        'err_proc_acor_phi_shape',
        'err_proc_acor_phi_rate',
        'err_proc_acor_sigma_rate',
        'err_proc_iid_sigma_rate'
      )
    )
  ...
  traceplot(
    get_mcmc(mm_new),
    pars = c("err_proc_acor_phi", "err_proc_acor_sigma", "err_proc_iid_sigma")
  )
  ...
  pairs(
    get_mcmc(mm_new),
    pars = c("GPP_daily[2]", "ER_daily[2]", "K600_daily[2]")
  )
  pairs(
    get_mcmc(mm_new),
    pars = c("err_proc_acor_phi", "err_proc_acor_sigma", "err_proc_iid_sigma")
  ...
  sp <- specs(
    ...
    revise(
      ...
      err_obs_iid_sigma_rate = 0.05,
      err_proc_iid_sigma_rate = 0.01,
      params_in = c(
        'GPP_daily_mu',
        'GPP_daily_sigma',
        'ER_daily_mu',
        ...
        'K600_daily_beta_sigma',
        'K600_daily_sigma_rate',
        'err_obs_iid_sigma_rate',
        'err_proc_iid_sigma_rate'
      ),
      params_out = setdiff(params_out, 'err_proc_iid')
    ) # a little faster f run without 'err_proc_iid' in params_out (but just 1 Mb)
  ...
  traceplot(
    get_mcmc(mm_new2),
    pars = c("K600_daily_beta", "K600_daily_sigma"),
    inc_warmup = F
  )
  pairs(
    get_mcmc(mm_new2),
  ...
  mm_fast <- sp %>%
    revise(
      ...
      delete = c('err_obs_iid_sigma_scale'),
      params_in = setdiff(
  ...
  sp2 <- sp %>%
    revise(
      ...
      K600_daily_beta_sigma = c(intercept = 0.3, slope = 0.3),
      K600_daily_sigma_rate = 1,
      err_obs_iid_sigma_rate = 0.1,
      delete = c(
        "K600_daily_sigma_location",
  ...
  mm_new2linlog <- metab(
    revise(sp2, model_name = 'inst/models/b_Kl_oi_pm_plrckm_sfs2linlog.stan'),
    data = dat
  ) # 18 sec
  mm_new2loglog <- metab(
    revise(sp2, model_name = 'inst/models/b_Kl_oi_pm_plrckm_sfs2loglog.stan'),
    data = dat
  ) # 18 sec
  mm_new3linlog <- metab(
    revise(sp3, model_name = 'inst/models/b_Kl_oi_pm_plrckm_sfs2linlog.stan'),
    data = dat
  ) # 21 sec
  mm_new3loglog <- metab(
    revise(sp3, model_name = 'inst/models/b_Kl_oi_pm_plrckm_sfs2loglog.stan'),
    data = dat
  ) # 53 sec. good priors matter!
}
...
useful_code <- function() {
  ...
  debug_metab <- function(specs) {
    ...
    specs$model_path <- system.file(
      paste0("models/", specs$model_name),
    ...
    system.time({
      suppressWarnings(
        {
          mcmc_out <- do.call(
            streamMetabolizer:::mcmc_bayes,
            c(list(data_list = data_list, keep_mcmc = TRUE), specs[mcmc_args])
          )
        }
      )
    })
