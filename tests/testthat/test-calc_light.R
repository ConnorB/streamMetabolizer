test_that("high-level light function works", {
  stimes <- lubridate::force_tz(
    as.POSIXct(sprintf('2016-09-27 %02d:00', 1:24)),
    'UTC'
  )
...
test_that("can generate light predictions from basic light model", {
  ...
  decdf <- data.frame(
    ...
    dec_rad = streamMetabolizer:::calc_declination_angle(
      0:365,
      format = "radia"
    )
  )
  ...
  solstice <- which(
    decdf$jday == as.numeric(format(as.Date("2015-06-21"), "%j"))
  )
  expect_gt(
    decdf[solstice, "dec"],
    decdf[equinox, "dec"],
    "solstice more declined than equinox"
  )
  expect_equal(
    max(decdf$dec),
    -1 * (min(decdf$dec)),
    info = "equal & opposite declination on summer/winter solstices"
  )
  expect_equal(
    streamMetabolizer:::to_radians(decdf$dec),
    decdf$dec_rad,
    info = "degree & radian formats agree"
  )
  ...
  expect_equal(
    hourdf[hourdf$hour == 12, "hragl"],
    0,
    info = "hour angle is 0 at solar noon"
  )
  expect_equal(
    -1 * hourdf[0:13, "hragl"],
    hourdf[25:13, "hragl"],
    info = "hour angles reverse at noon"
  )
  expect_equal(
    streamMetabolizer:::to_radians(hourdf$hragl),
    streamMetabolizer:::calc_hour_angle(0:24, format = "radians")
  )
  ...
  insdf <- transform(
    insdf,
    datetime = convert_UTC_to_solartime(
...
test_that("calc_solar_insolation has consistent output with that of calc_sun_rise_set and calc_is_daytime", {
  ...
  insdf <- data.frame(
    ...
    mutate(
      ...
      app.solar.time = as.POSIXct(
        strptime(sprintf("2000-%d 00", jday), format = "%Y-%j %H"),
        tz = "UTC"
      ),
  ...
  expect_lt(
    max(filter(diffs, lat == 0)[, c("max_diff_sunrise", "max_diff_sunset")]),
    3.5
  )
  expect_lt(
    max(filter(diffs, lat == 20)[, c("max_diff_sunrise", "max_diff_sunset")]),
    5
  )
  expect_lt(
    max(filter(diffs, lat == 40)[, c("max_diff_sunrise", "max_diff_sunset")]),
    7.5
  )
  expect_lt(
    max(filter(diffs, lat == 60)[, c("max_diff_sunrise", "max_diff_sunset")]),
    12
  )
  expect_lt(
    max(filter(diffs, lat == 80)[, c("max_diff_sunrise", "max_diff_sunset")]),
    100
  ) # difference of up to 97 minutes at high lat when run for all days
})
