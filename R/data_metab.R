...
data_metab <- function(
  ...
  if (lifecycle::is_present(attach.units)) {
    ...
    if (isTRUE(attach.units)) {
      lifecycle::deprecate_warn(
        "0.12.0",
        "streamMetabolizer::data_metab(attach.units)"
      )
    }
  }
  ...
  if ('missing end' %in% flaws) {
    skip_rows <- c(
      ...
      switch(
        ...
        '3' = day3_start + 1:24,
        '10' = day_length +
          c(day3_start - 1:42, (day3_start + 2 * day_length) - 1:18)
      )
    )
  }
  ...
  if ('missorted' %in% flaws) {
    swap_rows <- switch(
      ...
      '10' = list(
        from = c(day3_start + 71:130, (day3_start + 2 * day_length) + 14:55),
        to = day2_start + 45
      )
    )
