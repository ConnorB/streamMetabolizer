...
metab_mle <- function(
  ...
  fitting_time <- system.time({
    ...
    mle_all <- mm_model_by_ply(
      ...
      specs = specs
    ) # for mle_1ply and create_calc_dDOdt
  })
...
mle_1ply <- function(
  ...
  if (length(stop_strs) == 0) {
      ...
      if (nrow(data_daily_ply) == 0) {
        if (length(daily.inits) > 0) {
          warn_strs <- c(
            warn_strs,
            paste0(
              "nrow(data_daily)==0 so using specs for ",
              paste0(daily.inits, collapse = ', ')
            )
          )
        }
        if (exists('K600', data_daily_ply)) {
          warn_strs <- c(
            warn_strs,
            "nrow(data_daily)==0 so fitting K600 by MLE"
          )
        }
      } else {
        # nrow(data_daily_ply)==1 because we're in mle_1ply, so it can only be 0 or 1
        for (init in daily.inits) {
          if (is.na(data_daily_ply[[init]])) {
            warn_strs <- c(
              warn_strs,
              paste0("data_daily$", init, "==NA so using specs")
            )
          } else {
        ...
        if (exists('K600.daily', data_daily_ply)) {
          if (is.na(data_daily_ply$K600.daily)) {
            warn_strs <- c(
              warn_strs,
              "data_daily$K600.daily==NA so fitting by MLE"
            )
          } else {
    ...
    if (fix_K600) {
      init.vals <- init.vals[names(init.vals) != 'init.K600.daily']
    }
    ...
    NLL <- create_calc_NLL(
      DO,
      err_obs_iid = features$err_obs_iid,
  ...
  valstat.names <- paste0(
    rep(val.names, each = length(stat.names)),
    rep(c('', '.sd', '.grad'), times = length(val.names))
  ...
  if (length(stop_strs) > 0) {
    ...
    if (goodness.cols$code > 2) {
      warn_strs <- c(warn_strs, goodness.cols$code.str)
    }
  }
  err.cols <- data.frame(
