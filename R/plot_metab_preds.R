...
#' @import dplyr
#' @export
plot_metab_preds <- function(
  metab_preds,
  y_var = c('GPP', 'ER'),
  style = c('ggplot2'),
  y_lim = list(GPP = c(NA, NA), ER = c(NA, NA))
) {
  if (is(metab_preds, 'metab_model')) {
    metab_preds <- predict_metab(metab_preds)
  }
  ...
  metab_preds_GPP <- mutate(
    ...
    upr = GPP.upper,
    var = 'GPP (g m^-2 d^-1)',
    col1 = params$colors[['GPP']][1],
    col2 = params$colors[['GPP']][2],
    lab = 'GPP~(g~m^-2~d^-1)'
  )
  metab_preds_ER <- mutate(
    ...
    upr = ER.upper,
    var = 'ER (g m^-2 d^-1)',
    col1 = params$colors[['ER']][1],
    col2 = params$colors[['ER']][2],
    lab = 'ER~(g~m^-2~d^-1)'
  )
      ...
      if (any(!is.na(preds_ggplot$lwr) & !is.na(preds_ggplot$upr))) {
        g <- g +
          ggplot2::geom_errorbar(
            ggplot2::aes(ymin = lwr, ymax = upr, color = col1),
            alpha = 0.5
          )
      }
      g <- g +
