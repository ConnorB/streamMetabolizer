...
#' @returns A POSIXct object that says it's in tz="UTC" but that's actually in
...
convert_UTC_to_solartime <- function(
  ...
  time.adjustment <- 3.989 * longitude # minutes
  ...
  if (time.type == "mean solar") {
    ...
    E <- 9.87 *
      sin(to_radians((2 * 360 * (jday - 81)) / 365)) -
      7.53 * cos(to_radians((360 * (jday - 81)) / 365)) -
      1.5 * sin(to_radians((360 * (jday - 81)) / 365)) # Equation of time as in Yard et al. 2005
...
#' @returns A POSIXct object in UTC.
#' @export
#' @references Yard, Bennett, Mietz, Coggins, Stevens, Hueftle, and Blinn. 2005.
#'   Influence of topographic complexity on solar insolation estimates for the
#'   Colorado River, Grand Canyon, AZ. Ecological Modelling.
...
convert_solartime_to_UTC <- function(
  ...
  if (!(tz(any.solar.time) %in% c("GMT", "Etc/GMT-0", "Etc/GMT+0", "UTC"))) {
    .cli_abort("{.var solar.time} must use the nominal {.val UTC} time zone.")
  }

  conversion <- any.solar.time -
    convert_UTC_to_solartime(any.solar.time, longitude, time.type)
