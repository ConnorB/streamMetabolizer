...
plot_DO_preds <- function(
  ...
  date_start = NA,
  date_end = NA,
  use_saved = TRUE # these inherited from predict_DO
) {
  ...
  DO_preds_conc <- mutate(
    ...
    var = 'DO (mg/L)',
    lab = 'DO (mg/L)',
    col.pure = params$colors$conc[1],
    col.mod = params$colors$conc[2],
    col.obs = params$colors$conc[3],
  ...
  DO_preds_pctsat <- mutate(
    ...
    var = 'DO (% sat)',
    lab = 'DO (% sat)',
    col.pure = params$colors$pctsat[1],
    col.mod = params$colors$pctsat[2],
    col.obs = params$colors$pctsat[3],
  ...
  DO_preds_ddodt <-
    mutate(
      ...
      as = 'ddodt',
      var = 'dDO/dt (mg/L/d)',
      lab = 'dDO/dt~(mg~L^-1~d^-1)',
      col.pure = params$colors$ddodt[1],
      col.mod = params$colors$ddodt[2],
      col.obs = params$colors$ddodt[3],
      pure = if (exists('DO.pure', DO_preds)) {
        diff(DO_preds$DO.pure) /
          as.numeric(diff(DO_preds$solar.time), units = "days")
      } else {
        NA
      },
      mod = diff(DO_preds$DO.mod) /
        as.numeric(diff(DO_preds$solar.time), units = "days"),
  ...
  DO_preds_all <- bind_rows(DO_preds_conc, DO_preds_pctsat, DO_preds_ddodt) %>%
    mutate(
      var = ordered(
        var,
        c(conc = 'DO (mg/L)', pctsat = 'DO (% sat)', ddodt = 'dDO/dt (mg/L/d)')[
          y_var
        ]
      )
    )
  ...
  plot_out <- switch(
    ...
    'ggplot2' = {
      ...
      if ('conc' %in% names(y_lim)) {
        ...
        if (!is.na(lim)) {
          preds_ggplot <- filter(
            preds_ggplot,
            as != 'conc' | (pure >= lim & mod >= lim & obs >= lim)
          )
        }
        lim <- y_lim[['conc']][2]
        if (!is.na(lim)) {
      ...
      if ('pctsat' %in% names(y_lim)) {
        ...
        if (!is.na(lim)) {
          preds_ggplot <- filter(
            preds_ggplot,
            as != 'pctsat' | (pure >= lim & mod >= lim & obs >= lim)
          )
        }
        lim <- y_lim[['pctsat']][2]
        if (!is.na(lim)) {
      ...
      if ('ddodt' %in% names(y_lim)) {
        ...
        if (!is.na(lim)) {
          preds_ggplot <- filter(
            preds_ggplot,
            as != 'ddodt' | (pure >= lim & mod >= lim & obs >= lim)
          )
        }
        lim <- y_lim[['ddodt']][2]
        if (!is.na(lim)) {
      ...
      g <- ggplot2::ggplot(
        preds_ggplot,
        ggplot2::aes(x = solar.time, group = date)
      )
      # optional (only applies to sim models): 'pure' lines
      if (any(!is.na(preds_ggplot$pure))) {
        g <- g +
          ggplot2::geom_line(
            ggplot2::aes(y = pure, color = col.pure),
            linewidth = 0.8,
            na.rm = TRUE
          )
      }
      g +
        ggplot2::geom_point(
          ggplot2::aes(y = obs, color = col.obs),
          alpha = 0.6,
          na.rm = TRUE
        ) +
        ggplot2::geom_line(
          ggplot2::aes(y = mod, color = col.mod),
          linewidth = 0.8,
          na.rm = TRUE
        ) +
    ...
    'dygraphs' = {
      ...
      ymin <- max(
        c(min(c(unclass(dat)), na.rm = TRUE), y_lim[[y_var]][1]),
        na.rm = TRUE
      )
      ymax <- min(
        c(max(c(unclass(dat)), na.rm = TRUE), y_lim[[y_var]][2]),
        na.rm = TRUE
      )
      d <- dygraphs::dygraph(
        dat,
