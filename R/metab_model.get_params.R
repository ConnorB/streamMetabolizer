...
get_params.metab_model <- function(
  ...
  if (
    length(
      missing.metabs <- param.names$required[
        !param.names$required %in% union(names(fit), names(ddat))
      ]
    ) >
      0
  ) {
  ...
  for (a in metab.either) {
    if (a %in% metab.both) {
      ...
      pars[[paste0(a, '.fixed')]] <- coalesce(
        ifelse(is.na(pars[[a.x]]), NA, FALSE),
        ifelse(is.na(pars[[a.y]]), NA, TRUE)
      )
    } else {
  ...
  if (uncertainty != 'none') {
    ...
    metab.uncert <- matrix(
      paste0(
        rep(metab.out, each = length(suffixes)),
        rep(suffixes, times = length(metab.out))
      ),
  ...
  if (fixed == 'stars') {
    params <- bind_cols(
      select(params, date),
      format.data.frame(select(params, -date))
    )
    for (a in metab.either) {
      isfixed <- params[[paste0(a, '.fixed')]]
      params[[a]] <- paste0(
        params[[a]],
        ifelse(is.na(isfixed), '?', ifelse(isfixed, '*', ' '))
      )
