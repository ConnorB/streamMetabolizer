...
load_spring_creek <- function(attach.units = deprecated()) {
  ...
  if (lifecycle::is_present(attach.units)) {
    ...
    if (isTRUE(attach.units)) {
      lifecycle::deprecate_warn(
        "0.12.0",
        "streamMetabolizer::load_spring_creek(attach.units)"
      )
    }
  }
  attach.units <- FALSE
  ...
  spring <- read.csv(file.name, stringsAsFactors = FALSE, header = TRUE) |>
    ...
    mutate(
      ...
      app.solar.time = convert_UTC_to_solartime(
        utc.time,
        longitude = -105.6,
        time.type = 'apparent solar'
      ),
      light = convert_SW_to_PAR(calc_solar_insolation(
        app.solar.time = app.solar.time,
        latitude = 41.33,
