...
mm_sd_to_ci <- function(data, alpha = 0.05) {
  ...
  var.cols <- substring(sd.cols, 1, nchar(sd.cols) - 3) |>
    (\(x) x[x %in% names(data)])() # could be fewer than sd.cols
  if (length(sd.cols) > 0) {
    sd.cols <- paste0(var.cols, '.sd')
  } # only include those cols that both end in '.sd' and have a paired var.cols value
  ...
  for (i in rev(seq_along(var.cols))) {
    ...
    dat.list <- append(
      ...
      setNames(
        ...
        paste0(var.cols[i], c('.lower', '.upper'))
      ),
      after = sd.pos[i]
    )
