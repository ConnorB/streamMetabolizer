...
mm_is_valid_day <- function(
  ...
  day_start = 4,
  day_end = 27.99, # inheritParams mm_model_by_ply
  day_tests = c(
    'full_day',
    'even_timesteps',
    'complete_data',
  ...
  # check input
  if (
    !missing(day_tests) && length(day_tests) == 0 && is.na(required_timestep[1])
  ) {
    return(TRUE)
  }
  ...
  if (
    any(c('full_day', 'even_timesteps') %in% day_tests) ||
      !isTRUE(is.na(required_timestep))
  ) {
  ...
  if ('full_day' %in% day_tests & is.finite(timestep.days)) {
    ...
    similar_time <- function(a, b, tol) {
      abs(as.numeric(a, units = "days") - as.numeric(b, units = "days")) <
        as.numeric(tol, units = "days")
    }
    if (
      !similar_time(
        min(data_ply$solar.time) - date_start,
        day_start,
        tol = timestep.days
      )
    ) {
    ...
    if (
      !similar_time(
        max(data_ply$solar.time) - date_start,
        day_end,
        tol = timestep.days
      )
    ) {
  ...
  if (!isTRUE(is.na(required_timestep)) && is.finite(timestep.days)) {
    ...
    if (
      !isTRUE(all.equal(
        timestep.days,
        required_timestep,
        tolerance = 0.002 * required_timestep
      ))
    ) {
      stop_strs <- c(
        stop_strs,
        sprintf("timestep != %0.08f d", required_timestep)
      )
    }
  }
