...
metab_Kmodel <- function(
  ...
  fitting_time <- system.time({
    ...
    if (length(specs$weights) > 0) {
      specs$weights <- match.arg(specs$weights, choices = c("1/CI", "K600/CI"))
    }
    if (length(specs$predictors) > 0) {
      specs$predictors <- match.arg(
        specs$predictors,
        choices = c("date", "velocity.daily", "discharge.daily"),
        several.ok = TRUE
      )
    }
    ...
    if (
      !(!('K600' %in% names(specs$transforms)) ||
...
prepdata_Kmodel <- function(
  ...
  if (!is.null(data) && nrow(data) > 0) {
    ...
    names(aggs_daily)[match(columns, names(aggs_daily))] <- paste0(
      columns,
      '.daily'
    )
  ...
  if (('CI.max' %in% names(filters)) && !isTRUE(is.na(filters[['CI.max']]))) {
    data_daily <- data_daily %>%
      mutate(
        weight = weight *
          if (
            K600.daily.upper.obs - K600.daily.lower.obs <= filters[['CI.max']]
          ) {
            1
          } else {
            0
...
Kmodel_allply <- function(
  ...
  # add transformations
  names(transforms) <- replace(
    names(transforms),
    names(transforms) == "K600",
    "K600.daily.obs"
  )
  ...
  switch(
    ...
    'mean' = {
      ...
      Kobs <- eval(
        parse(text = trans_preds[['K600.daily.obs']]),
        envir = data_daily_all
      )
      overall <- list(
    ...
    'lm' = {
      ...
      formul <- formula(paste0(
        ...
        " ~ ",
        paste0(trans_preds[-1], collapse = " + ")
      ))
      wts <- if (length(weights) > 0) data_daily_all$weight else NULL
      overall <- do.call(
        lm,
        c(
          list(formula = formul, data = data_daily_all, weights = wts),
          other_args
        )
      )
    },
    'loess' = {
      ...
      if (length(weights) > 0) {
        ...
        overall <- do.call(
          loess,
          c(list(formula = formul, data = data_daily_all), other_args)
        )
      }
    }
  )
...
get_params.metab_Kmodel <- function(
      ...
      loess = {
        ...
        params <- params %>%
          mutate(
            K600.daily = {
              preds$fit
            }, # {} avoids "Error: invalid subscript type 'list'"
            K600.daily.sd = {
              preds$se.fit
            }
          )
  ...
  metab_model@fit <- c(
    list(daily = mutate(params, warnings = '', errors = '')),
    get_fit(metab_model)
  ) # temporarily assign to @fit for compatibility with get_params.metab_model
...
#' @export
predict_DO.metab_Kmodel <- function(
  metab_model,
  date_start = NA,
  date_end = NA,
  ...,
