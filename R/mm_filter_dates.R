...
mm_filter_dates <- function(
  ...
  if (
    is.null(data) ||
      nrow(data) == 0 ||
      (is.character(data) && data == "generic metab_model class; no actual fit")
  ) {
    return(data)
  }
  date_col <- unlist(sapply(
    c('solar.time', 'date'),
    grep,
    names(data),
    fixed = TRUE,
  ...
  tidy_date <- function(date) {
    ...
    switch(
      date_col,
      'solar.time' = as.POSIXct(
        format(loc.date, date_format),
        format = date_format,
        tz = local.tz
      ),
      'date' = loc.date
    ...
    'solar.time' = {
      ...
      data |>
        dplyr::filter(
          ...
          if (!is.na(date_end)) {
            solar.time <= (date_end + as.difftime(day_end, units = "hours"))
          } else {
            TRUE
          }
        )
