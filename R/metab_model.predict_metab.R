...
predict_metab.metab_model <- function(
  ...
  if (lifecycle::is_present(attach.units)) {
    lifecycle::deprecate_warn(
      "0.12.0",
      "streamMetabolizer::predict_metab(attach.units)"
    )
  } else {
    attach.units <- FALSE
  }
  ...
  if (isTRUE(use_saved) && !is.null(metab_model@metab_daily)) {
    ...
    if (
      (!missing(day_start) && day_start != get_specs(metab_model)$day_start) ||
        (!missing(day_end) &&
          day_end != min(day_start + 24, get_specs(metab_model)$day_end))
    ) {
      .cli_warn(
    ...
    day_tests_pred <- if (mm_parse_name(specs$model_name)$type == 'sim') {
      specs$day_tests
    } else {
      c()
    }
    req_ts_pred <- if (mm_parse_name(specs$model_name)$type == 'sim') {
      specs$required_timestep
    } else {
      NA
    }
    preds <- mm_model_by_ply(
      ...
      day_start = day_start,
      day_end = day_end,
      day_tests = day_tests_pred,
      required_timestep = req_ts_pred,
      timestep_days = FALSE, # for mm_model_by_ply
    ...
    preds.cols <- append(
      names(preds),
      'msgs.fit',
      after = which(names(preds) == 'warnings') - 1
    )
