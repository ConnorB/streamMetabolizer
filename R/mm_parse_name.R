...
mm_parse_name <- function(model_name, expand = FALSE) {
  ...
  match_or_NA <- function(key, pairs) {
    ...
    if (length(matches) == 0) {
      ...
      .cli_abort("Found too many matches while parsing the PRK terms.")
    } else {
      matches
    }
  }
  ...
  sapply(1:length(parsed), function(pnum) {
    if (length(parsed[[pnum]]) <= 5) {
      .cli_abort(
        "Model name {.val {model_name[pnum]}} is missing one or more pieces."
      )
    }
  })
  type <- unname(c(
  ...
  prk_terms <- bind_rows(lapply(parsed, function(parsed1) {
    prk_term <- parsed1[5]
    prk_pairs <- if (nchar(prk_term) == 0) {
      c()
    } else {
      sapply(seq(2, nchar(prk_term), by = 2), function(pos) {
        substring(prk_term, pos - 1, pos)
      })
    }
    data.frame(
      deficit_src = match_or_NA(
        c(km = 'DO_mod', ko = 'DO_obs', kf = 'DO_obs_filter'),
        prk_pairs
      ),
  ...
  if (!expand) {
    df$model_name <- df$pool_K600_type <- df$pool_K600_sd <- NULL
  }
