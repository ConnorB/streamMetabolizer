...
metab_night <- function(
  ...
  fitting_time <- system.time({
    ...
    night_all <- mm_model_by_ply(
      ...
      day_end = specs$day_end, # for mm_model_by_ply
      day_tests = c(),
      required_timestep = NA,
      timestep_days = FALSE, # for mm_model_by_ply - bypass the regular timestep calc & validity check on the full day ply
      night_tests = specs$day_tests,
      specs = specs
    ) # for nightreg_1ply
  })
...
nightreg_1ply <- function(
    ...
    tryCatch(
      {
        ...
        date_start <- as.POSIXct(
          paste0(as.character(ply_date), " 00:00:00"),
        ...
        validity <- mm_is_valid_day(
          ...
          day_end = night_end,
          day_tests = night_tests,
          required_timestep = specs$required_timestep,
          ply_date = ply_date
        ) # args passed from metab_night (after modifying the tests)
        if (!isTRUE(validity)) {
          stop_strs <- c(stop_strs, validity)
        }
        ...
        # smooth DO data
        night_dat$DO.obs.smooth <- c(stats::filter(
          night_dat$DO.obs,
          rep(1 / 3, 3),
          sides = 2
        ))
        ...
        out <- list(
          ...
          ER.vol.sd = summary(lm_dDOdt)$coefficients[[
            "(Intercept)",
            "Std. Error"
          ]],
        ...
        out <- c(
          out,
          list(
            K600.daily = convert_kGAS_to_k600(
              kGAS = out$KO2,
              temperature = mean(night_dat$temp.water),
              gas = "O2"
            ),
            K600.daily.sd = convert_kGAS_to_k600(
              kGAS = out$KO2.sd,
              temperature = mean(night_dat$temp.water),
              gas = "O2"
            )
  ...
  out.cols <- c(
    ...
    'row.first',
    'row.last'
  ) # could also include 'KO2','KO2.sd','ER.vol','ER.vol.sd'
  ...
  data.frame(
    select(as.data.frame(night.1d), all_of(out.cols)),
...
#' @export
#' @import dplyr
predict_DO.metab_night <- function(
  metab_model,
  date_start = NA,
  date_end = NA,
  ...,
  ...
  mm_model_by_ply(
    model_fun = metab_night_predict_1ply,
    data = data,
    data_daily = metab_ests, # for mm_model_by_ply
    day_start = day_start,
    day_end = day_end,
    day_tests = c(),
...
metab_night_predict_1ply <- function(
  ...
  if (
    nrow(data_daily_ply) > 0 &&
      !is.na(data_daily_ply$row.first) &&
      !is.na(data_daily_ply$row.last)
  ) {
    which_night <- seq(
      min(nrow(data_ply), data_daily_ply$row.first),
  ...
  mm_predict_DO_1ply(
    data_ply = night_dat,
    data_daily_ply = select(data_daily_ply, date, ER.daily, K600.daily),
    day_start,
    day_end,
    ply_date,
