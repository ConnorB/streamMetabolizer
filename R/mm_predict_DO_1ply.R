...
mm_predict_DO_1ply <- function(
  ...
  if (features$type == 'sim') {
    ...
    n <- nrow(data_ply)
    err.obs <- as.numeric(stats::filter(
      rnorm(n, 0, data_daily_ply$err.obs.sigma),
      filter = data_daily_ply$err.obs.phi,
      method = "recursive"
    ))
    err.proc <- as.numeric(stats::filter(
      rnorm(n, 0, data_daily_ply$err.proc.sigma),
      filter = data_daily_ply$err.proc.phi,
      method = "recursive"
    ))
    errs <- list(
      ...
      list(colname = 'DO.obs', err.obs = err.obs, err.proc = err.proc)
    ) # the 'observed' DO (with obs err)
  } else {
    errs <- list(
