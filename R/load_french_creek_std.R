...
#'   data.frame?
#' @returns A data.frame, unitted if attach.units==TRUE.
load_french_creek_std <- function(attach.units = deprecated()) {
  ...
  if (lifecycle::is_present(attach.units)) {
    ...
    if (isTRUE(attach.units)) {
      lifecycle::deprecate_warn(
        "0.12.0",
        "streamMetabolizer::load_french_creek_std(attach.units)"
      )
    }
  }
  attach.units <- FALSE
  ...
  ) # L10 data from French Creek, Hotchkiss and Hall, In press, Ecology
  ...
  french$dtime <- chron::chron(
    dates = as.character(french$date),
    times = as.character(french$time)
  ) # L22. TZ is MST (L108, 142)
  ...
  french$local.time <- with_tz(
    ...
    tz_french
  ) # need POSIXct for streamMetabolizer
  ...
  bpcalc <- function(bpst, alt, temp) {
    ...
    bpst *
      25.4 *
      exp((-9.80665 * 0.0289644 * alt) / (8.31447 * (273.15 + temp)))
...
load_french_creek_std_mle <- function(
  french,
  K = 35,
  estimate = c('PRK', 'K', 'PR'),
  ...
  french$dtime <- chron::chron(
    format(french$solar.time, "%m/%d/%y"),
    times = format(french$solar.time, "%H:%M:%S")
  )
  o2file <- french[
    french$dtime >= as.numeric(start) & french$dtime <= as.numeric(end),
  ]
    ...
    'PRK' = {
      rivermetabK <- function(o2file, z, bp, ts) {
        ...
        onestationmleK <- function(MET, temp, oxy, light, z, bp, ts) {
          ...
          for (i in 2:length(oxy)) {
            metab[i] <- metab[i - 1] +
              ...
              (Kcor(temp[i - 1], MET[3])) *
                ts *
                (osat(temp[i - 1], bp) - metab[i - 1])
          }
          # calculate likelihood
          sqdiff <- (oxy - metab)^2
          length(oxy) *
            (log(((sum(sqdiff) / length(oxy))^0.5)) + 0.5 * log(2 * pi)) +
        ...
        river.mle <- nlm(
          ...
          oxy = o2file$DO.obs,
          z = z,
          temp = o2file$temp.water,
          light = o2file$light,
          bp = bp,
          ts = ts
        )
        data.frame(
          GPP = river.mle$estimate[1],
          ER = river.mle$estimate[2],
          K = river.mle$estimate[3],
          lik = river.mle$minimum[1]
        )
      }
    ...
    'K' = {
      nightreg <- function(o2file, bp, ts) {
        ...
        K600fromO2 <- function(temp, KO2) {
          # function to estimate K600 from KO2  needed for nightime regression
          #((600/(1800.6 - (120.1 * temp) + (3.7818 * temp^2) - (0.047608 * temp^3)))^-0.5) * KO2 # bob & maite's original
          ((600 /
            (1568 - 86.04 * temp + 2.142 * temp^2 - 0.0216 * temp^3))^-0.5) *
            KO2 # to match raymond et al. 2012
        }
        data.frame(
          GPP = as.numeric(NA),
          ER = NA,
          K = K600fromO2(mean(o2file$temp.water), coef(nreg)['satdef']),
          lik = as.numeric(NA)
        )
      }
