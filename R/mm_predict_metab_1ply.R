...
mm_predict_metab_1ply <- function(
  ...
  preds <- bind_cols(lapply(c('GPP', 'ER'), function(met) {
    # lapply can also apply to 'D', but leaving out for now
            ...
            mean(mapply(
              function(t, DO.mod.t) {
                switch(
                  met,
                  GPP = ,
                  ER = env.dDOdt[[met]](t = t, metab.pars = data_daily_ply),
                  D = env.dDOdt[[met]](
                    t = t,
                    metab.pars = data_daily_ply,
                    DO.mod.t = DO.mod.t
                  ) *
                    env.dDOdt$data$depth[t]
                )
              },
              t = t,
              DO.mod.t = data_ply$DO.mod
            )),
