...
mm_generate_mcmc_file_deprecated <- function(
  ...
  model_text <- c(
    ...
    c(
      ...
      chunk(
        ...
        if (pool_K600_type %in% c('normal', 'linear', 'binned')) {
          c(
            p(''),
            comment(
              'Parameters of hierarchical priors on K600_daily (',
              pool_K600,
              ' model)'
            )
          )
        ...
        switch(
          ...
          fixed = switch(
            ...
            linear = ,
            binned = 'real<lower=0> K600_daily_sigma;'
          ),
          fitted = switch(
            pool_K600_type,
            normal = 'real<lower=0> K600_daily_sdlog_sigma;',
            linear = ,
            binned = 'real<lower=0> K600_daily_sigma_sigma;'
          )
        )
      ),
    ...
    c(
      ...
      indent(
        ...
        switch(
          ...
          satlight = c('vector[d] alpha_scaled;', 'vector[d] Pmax;') #<lower=0>
        ),
        c(
          'vector<upper=ER_daily_upper>[d] ER_daily;',
          if (pool_K600_sd %in% c('fixed', 'fitted')) {
            c(
        ...
        if (pool_K600_type != 'none') {
          c(
            ...
            if (pool_K600_sd == 'fitted') {
              switch(
                pool_K600_type,
                normal = 'real<lower=0> K600_daily_sdlog_scaled;',
                linear = ,
                binned = 'real<lower=0> K600_daily_sigma_scaled;'
              )
            }
          )
        },
    ...
    chunk(
      ...
      if (pool_K600_type %in% c('linear', 'binned')) {
        c(
          'vector[d] K600_daily_predlog;'
        )
      },
      if (pool_K600_sd == 'zero') {
        c(
          'vector[d] K600_daily;'
        )
      },
      ...
      if (err_proc_iid) {
        ...
        # err_obs_iid and/or err_proc_acor without err_proc_iid
        'vector[d] DO_mod[n];'
      },
      if (err_proc_acor) {
        sprintf(
          'vector[d] err_proc_acor[%s];',
          switch(ode_method, euler = 'n-1', trapezoid = 'n')
        )
      }
    ),
    ...
    if (pool_K600_sd == 'fitted') {
      chunk(
        ...
        switch(
          ...
          linear = ,
          binned = s(fs('halfnormal', 'K600_daily_sigma'))
        )
      )
    },
    ...
    # K600_daily model
    if (pool_K600_type %in% c('linear', 'binned') || pool_K600_sd == 'zero') {
      chunk(
    ...
    indent(
      ...
      comment(
        ...
        paste0(
          c(
            if (err_proc_iid) 'IID',
            if (err_proc_acor) 'autocorrelated',
            if (!err_proc_iid && !err_proc_acor) 'no'
          ),
          collapse = ' and '
        ),
        ' process error'
      ...
      if (err_proc_acor) {
        c(
          ...
          s(
            '  err_proc_acor[i] = err_proc_acor_phi * err_proc_acor[i-1] + err_proc_acor_inc[i]'
          ),
          p('}')
        )
      },
          ...
          indent(
            switch(
              ...
              'trapezoid' = c(
                switch(
                  ...
                  'DO_mod' = c(
                    p(
                      if (err_obs_iid) 'DO_mod' else 'DO_mod_partial',
                      '[i] .*'
                    ),
                    p(
            ...
            switch(
              ...
              'euler' = c(
                p(
                  '  KO2_inst[i] .* (DO_sat[i] - ',
                  if (deficit_src == 'DO_mod' && !err_obs_iid) {
                    'DO_mod_partial'
                  } else {
                    deficit_src
                  },
      ...
      chunk(
        if (err_proc_iid) {
          c(
            ...
            indent(
              s(
                if (!err_obs_iid) 'DO_obs[i]' else 'DO_mod[i]',
                ' ~ ',
                f(
                  'normal',
                  mu = 'DO_mod_partial[i]',
                  sigma = 'DO_mod_partial_sigma[i]'
                )
              )
            ),
            p('}')
    ...
    if (err_proc_GPP) {
      chunk(
        ...
        indent(
          s(
            'err_mult_GPP[i] ~ ',
            f('lognormal', meanlog = 0, sdlog = 'err_mult_GPP_sdlog')
          )
        ),
        p('}'),
    ...
    if (err_obs_iid) {
      chunk(
        ...
        if (err_obs_iid && err_proc_iid) {
          # only works for state-space models because these have DO_mod as param, not trans param
          p('for(i in 1:n) {')
        } else {
          p('for(i in 2:n) {')
        },
        indent(
          s(
            'DO_obs[i] ~ ',
            f('normal', mu = p('DO_mod[i]'), sigma = 'err_obs_iid_sigma')
          )
        ),
        p('}'),
    ...
    indent(
      ...
      switch(
        ...
        linlight = s(
          'GPP_daily ~ ',
          f('normal', mu = 'GPP_daily_mu', sigma = 'GPP_daily_sigma')
        ),
        satlight = c(
      ...
      s(
        'ER_daily ~ ',
        f('normal', mu = 'ER_daily_mu', sigma = 'ER_daily_sigma')
      ),
      if (pool_K600_sd %in% c('fixed', 'fitted')) {
        c(
          switch(
            ...
            none = s(
              ...
              f(
                'lognormal',
                meanlog = 'K600_daily_meanlog',
                sdlog = 'K600_daily_sdlog'
              )
            ),
            normal = s(
              'K600_daily ~ ',
            ...
            binned = s(
              ...
              f(
                'normal',
                mu = 'exp(K600_daily_predlog)',
                sigma = 'K600_daily_sigma'
              )
            )
          )
        )
    ...
    if (pool_K600_type != 'none') {
      chunk(
        comment(
          'Hierarchical constraints on K600_daily (',
          pool_K600,
          ' model)'
        ),
        switch(
          ...
          'normal' = c(
            s(
              ...
              f(
                'normal',
                mu = 'K600_daily_meanlog_meanlog',
                sigma = 'K600_daily_meanlog_sdlog'
              )
            )
          ),
          'linear' = c(
            s(
              ...
              f(
                'normal',
                mu = 'lnK600_lnQ_intercept_mu',
                sigma = 'lnK600_lnQ_intercept_sigma'
              )
            ),
            s(
              'lnK600_lnQ_slope ~ ',
              f(
                'normal',
                mu = 'lnK600_lnQ_slope_mu',
                sigma = 'lnK600_lnQ_slope_sigma'
              )
            )
          ),
          'binned' = c(
            s(
              ...
              f(
                'normal',
                mu = 'K600_lnQ_nodes_meanlog',
                sigma = 'K600_lnQ_nodes_sdlog'
              )
            ),
            p('for(k in 2:b) {'),
            s(
              ...
              f(
                'normal',
                mu = 'lnK600_lnQ_nodes[k-1]',
                sigma = 'K600_lnQ_nodediffs_sdlog'
              )
            ),
            p('}')
          )
        ...
        c(
          ...
          if (err_proc_iid) {
            c(
              ...
              indent(
                s(
                  'err_proc_iid[i-1] = (DO_mod_partial[i] - ',
                  if (!err_obs_iid) 'DO_obs[i]' else 'DO_mod[i]',
        ...
        if (err_obs_iid) {
          c(
            ...
            s(
              ...
              'sum((DO_obs_vec - mean(DO_obs_vec)) .* (DO_obs_vec - mean(DO_obs_vec)))'
            ) # sum((y - y_bar)^2)
          )
        } else {
          c(
  ...
  writeLines(
    model_text,
    con = paste0('inst/models/deprecated/', model_name),
    sep = "\n"
  )
}
