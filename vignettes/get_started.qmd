...
The fitting and inspection chunks are not evaluated during routine vignette
builds because Stan sampling is computationally expensive and can occasionally
fail for a short illustrative run. To execute them while rendering this
vignette, set the environment variable
`STREAMMETABOLIZER_RUN_STAN_VIGNETTE=true`.

```{r bayes_fit, eval=identical(tolower(Sys.getenv("STREAMMETABOLIZER_RUN_STAN_VIGNETTE")), "true")}
...
```{r bayes_show, eval=identical(tolower(Sys.getenv("STREAMMETABOLIZER_RUN_STAN_VIGNETTE")), "true")}
mm
```

Here are the daily metabolism predictions from the model:
```{r bayes_pred_tbl, eval=identical(tolower(Sys.getenv("STREAMMETABOLIZER_RUN_STAN_VIGNETTE")), "true")}
predict_metab(mm)
```

```{r bayes_pred_fig, fig.width=5, fig.height=5, eval=identical(tolower(Sys.getenv("STREAMMETABOLIZER_RUN_STAN_VIGNETTE")), "true")}
plot_metab_preds(mm)
```

You can inspect more of the fitted daily parameters, including K600, with `get_params()`:
```{r bayes_params, eval=identical(tolower(Sys.getenv("STREAMMETABOLIZER_RUN_STAN_VIGNETTE")), "true")}
get_params(mm)
```
...
```{r bayes_pdo_tbl, results='asis', eval=identical(tolower(Sys.getenv("STREAMMETABOLIZER_RUN_STAN_VIGNETTE")), "true")}
predict_DO(mm) |> head()
```

And here are the dissolved oxygen predictions in a figure:
```{r bayes_pdo_fig, fig.width=5, fig.height=5, eval=identical(tolower(Sys.getenv("STREAMMETABOLIZER_RUN_STAN_VIGNETTE")), "true")}
plot_DO_preds(mm)
```
...
```{r bayes_traceplot, fig.width=5, fig.height=5, eval=identical(tolower(Sys.getenv("STREAMMETABOLIZER_RUN_STAN_VIGNETTE")), "true")}
mcmc <- get_mcmc(mm)
...
```{r bayes_convergence, eval=identical(tolower(Sys.getenv("STREAMMETABOLIZER_RUN_STAN_VIGNETTE")), "true")}
get_fit(mm)$overall |>
  select(ends_with('Rhat'))
```

And here is a list of all column names available through `get_fit()`:
```{r bayes_fit_columns, eval=identical(tolower(Sys.getenv("STREAMMETABOLIZER_RUN_STAN_VIGNETTE")), "true")}
get_fit(mm) |>
  lapply(names)
```
