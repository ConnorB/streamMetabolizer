...
After fitting a `streamMetabolizer` model, retrieve the fitted Stan object with `get_mcmc(mm)`, where `mm` is your fitted metabolism model. With `stan_engine = "rstan"`, this is an RStan `stanfit` object. Useful first checks include `rstan::traceplot()`, `rstan::summary()`, `rstan::stan_diag()`, and `rstan::get_sampler_params()`; `inc_warmup = TRUE` is often useful when diagnosing a trace plot. See `?rstan::stanfit` for the full interface.

With `stan_engine = "cmdstanr"`, `get_mcmc(mm)` returns a `CmdStanMCMC` object; use its `$draws()`, `$summary()`, and `$diagnostic_summary()` methods.
