...
options(width=80)
run_bayesian_example <- identical(
  tolower(Sys.getenv("STREAMMETABOLIZER_RUN_STAN_VIGNETTE")),
  "true"
)
```
...
```{r bayes_fit, eval=run_bayesian_example}
...
```{r, eval=run_bayesian_example}
mm
```

Here are the daily metabolism predictions from the model:
```{r bayes_pred_tbl, eval=run_bayesian_example}
predict_metab(mm)
```

```{r bayes_pred_fig, fig.width=5, fig.height=5, eval=run_bayesian_example}
plot_metab_preds(mm)
```
...
```{r bayes_pdo_tbl, results='asis', eval=run_bayesian_example}
predict_DO(mm) %>% head()
```

And here are the dissolved oxygen predictions in a figure:
```{r bayes_pdo_fig, fig.width=5, fig.height=5, eval=run_bayesian_example}
plot_DO_preds(mm)
```
...
```{r, fig.width=5, fig.height=5, eval=run_bayesian_example}
mcmc <- get_mcmc(mm)
