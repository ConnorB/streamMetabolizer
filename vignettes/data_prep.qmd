...
Use `plot_metab_data()` to graphically inspect the input data. Look for outliers
and other oddities before fitting a model. By default, the function plots all
five measurement columns and calculates dissolved oxygen percent saturation.

```{r viz_inputs, fig.width=7, fig.height=8}
plot_metab_data(dat)
```
