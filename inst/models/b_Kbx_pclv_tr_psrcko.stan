...
model {
  ...
  if(d > 1) {
    for(j in 2:d) {
      0 ~ normal(err_proc_acor_phi * err_proc_acor[n-1,j-1], err_proc_acor_sigma + err_proc_acor_light_alpha * light_frac[1,j] + 1e-9);
    }
  }
  // Within each date, the first modeled residual follows the zero boundary residual
  err_proc_acor[1] ~ normal(0, err_proc_acor_sigma + err_proc_acor_light_alpha * light_frac[2] + 1e-9);
  for(i in 2:(n-1)) {
    err_proc_acor[i] ~ normal(err_proc_acor_phi * err_proc_acor[i-1], err_proc_acor_sigma + err_proc_acor_light_alpha * light_frac[i+1] + 1e-9);
  }
